...
void Play_background::minus_hp(Vector2D X, Bot* BOT)
{
    ...
    int wid = BOT->getWidth()/BOT->get_zoombot();
    int hei = BOT->getHeight()/BOT->get_zoombot();
    int id_X = X.getX();
    int id_Y = X.getY();
...
void Play_background::update()
{
    ...
    for(int i = 0; i < bot.size(); i++)
    {
        ...
        if(X->get_HP() <= 0)
        {
            ...
            int R = X->get_position().getX()- m_position.getX() - 640 + X->getWidth()/ X->get_zoombot();
            int L = R - X->getWidth()/ X->get_zoombot() ;
            if(R > 1165)
            {
                R = 1165;
                L = R - X->getWidth()/ X->get_zoombot() ;
            }
            if(L < 0)
            {
                L = 0;
                R = L + X->getWidth()/ X->get_zoombot() ;
            }
            if(X->get_zoombot() == 1)
        ...
        for(Bot* X:bot)
        {
            if(!X->get_is_running())
            {
                ...
                int val = X->get_position().getX()- m_position.getX() - 640 +
                          X->getWidth()/ X->get_zoombot() *(direction > 0) + 14 * direction;
                val = max(val, 0);
                val = min(val, 1165);
                if(X->get_zoombot() == 1 &&
            ...
            {
                int val = X->get_position().getX()- m_position.getX() - 640 +
                          (int)(1.0*X->getWidth()/ X->get_zoombot()) * (X->get_is_running() < 0) + 30 * (-X->get_is_running());
                val = max(val, 0);
                val = min(val, 1165);
                ...
                else if(X->get_countcf() == 1)
                {
                    ...
                    R = X->get_position().getX()- m_position.getX() - 640 + X->getWidth()/ X->get_zoombot();
                    L = R - X->getWidth()/ X->get_zoombot() ;
                    if(R > 1165)
                    {
                        R = 1165;
